...
export async function fetchExternalApi(sqlQuery = 'SELECT * FROM main LIMIT 10', fieldMap = {}) {
  try {
    const url = `${API_BASE_URL}/external_proxy.php?sql=${encodeURIComponent(sqlQuery)}`;
    const response = await fetch(url);
    if (response.ok) {
      const data = await response.json();
      if (Array.isArray(data) && data.length > 0) {
        return data.map(item => {
          const normalized = { ...item, _source: 'external_api' };
    ...
    console.warn("External API not reachable:", error);
  }
  return [];
}
